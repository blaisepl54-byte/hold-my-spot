...
export const WRITE_POOL_IMPORTERS: readonly { readonly path: string; readonly why: string }[] = [
  ...
  { path: "scripts/d2-join-flow-proof.ts", why: "teardown of the pool it drove through the HTTP routes" },
...
export const ENTRY_WRITERS_PERMITTED: readonly { readonly path: string; readonly why: string }[] = [
  ...
  { path: "scripts/d2-join-flow-proof.ts", why: "fixture teardown as hms_ddl; the entire join conversation is driven over real HTTP through the signed webhook, never by writing entries directly" },
